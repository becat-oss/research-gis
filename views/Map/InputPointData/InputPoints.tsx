...
export function drawLayer(
  ...
  return inputPointSet.map((pointData: InputPoint, index: number) => {
    return (
...
export default function InputPoints(): React.ReactElement {
  ...
  useEffect(() => {
    console.log("Conponent did mount layers from InputPoints", layers);
    ...
    <>
      {visibleLayers.map((layer) => {
        return (
          <div key={layer.name}>
            {drawLayer(
              inputPointSet.filter((obj) => layer.name.includes(obj.tag)),
              layer.color
            )}
